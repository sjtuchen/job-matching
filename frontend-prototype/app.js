...
const TYPE_META = {
  ...
  professional: { label: '专业技能', color: '#0f766e' },
  tool: { label: '工具', color: '#2563eb' },
...
const CONFIGURED_API = window.ZHIPU_API_BASE
  ? String(window.ZHIPU_API_BASE).replace(/\/+$/, '')
  : '';
const API_BASE = CONFIGURED_API || (location.port === '8000' ? '' : 'http://127.0.0.1:8000');
...
const state = {
  ...
  userUploaded: false,
  backendResumeId: 'S031',
  defaultFiles: [
    { name: '林知夏-简历.pdf', size: '1.2 MB', type: 'pdf', removable: true },
    { name: '成绩单.png', size: '860 KB', type: 'image', removable: true },
    { name: '自我介绍.mp4', size: '48 MB', type: 'video', removable: true }
  ]
};
...
function applyBackendPayload(payload) {
  if (!payload || !payload.profile || !Array.isArray(payload.jobs) || !payload.jobs.length) {
    return false;
  }
  const profile = payload.profile;
  CANDIDATE = {
    name: profile.name || '后端示例候选人',
    shortName: (profile.name || '候').slice(0, 1),
    school: [profile.school, profile.major].filter(Boolean).join(' · '),
    status: profile.degree || '本科',
    focus: profile.goal || profile.target_family || '新质岗位',
    summary: profile.summary || `${profile.school} ${profile.major}方向，已完成能力图谱抽取。`
  };

  PERSONAL_SKILLS = (payload.skills || []).map((skill) => ({
    id: skill.id || `s-${skill.name}`,
    name: skill.name,
    type: ['professional', 'tool', 'soft'].includes(skill.type) ? skill.type : 'hard',
    level: Number(skill.level) || 78,
    desc: skill.desc || skill.evidence || `能力图谱抽取项：${skill.name}`,
    evidence: [skill.evidence || skill.desc || '']
  }));

  const skillNameToId = Object.fromEntries(PERSONAL_SKILLS.map((skill) => [skill.name, skill.id]));
  PERSONAL_PROJECTS = (payload.projects || []).slice(0, 4).map((project) => ({
    id: project.id || `backend-project-${Math.random().toString(36).slice(2, 7)}`,
    name: project.name || project.summary?.slice(0, 20) || '项目经历',
    time: project.time || '在校项目',
  ...
  JOB_GROUPS = payload.jobs
    .filter((job) => job && job.title)
    .map((job, index) => ({
      ...job,
      key: job.key || job.id || `backend-job-${index}`,
      tags: job.tags?.length ? job.tags : (job.requirements?.required || []).slice(0, 3).map((item) => item.name),
      grade: job.grade || (job.score >= 60 ? '较匹配' : '可尝试'),
      path: job.path || [],
      gaps: job.gaps || [],
      dims: job.dims || [
        { label: '硬技能', mine: 70, target: 90 },
        { label: '软技能', mine: 70, target: 82 },
        { label: '项目经历', mine: 70, target: 84 },
        { label: '学历证书', mine: 78, target: 86 },
        { label: '行业知识', mine: 70, target: 88 },
        { label: '岗位契合', mine: job.score || 70, target: 100 }
      ]
    }));
  if (!JOB_GROUPS.length) return false;

  state.currentJob = JOB_GROUPS[0];
  state.backendAvailable = true;
  state.backendResumeId = profile.id || state.backendResumeId;
  state.defaultFiles = [];
...
async function connectToBackend() {
  if (state.userUploaded) return false;
  try {
    const payload = await fetchJson(`${API_BASE}/api/bootstrap?resume_id=${state.backendResumeId}&top=3`, 3000);
    if (state.userUploaded) return false;
...
function rememberUploadedSession(payload) {
  try {
    localStorage.setItem('zhipu-uploaded-session', JSON.stringify(payload));
  } catch (error) {
    // 浏览器禁用本地存储时忽略
  }
}

function restoreUploadedSession() {
  try {
    const raw = localStorage.getItem('zhipu-uploaded-session');
    if (!raw) return null;
    const payload = JSON.parse(raw);
    if (!payload?.profile || !payload?.jobs?.length) return null;
    const applied = applyBackendPayload(payload);
    if (applied) {
      state.userUploaded = true;
      state.backendAvailable = true;
      state.backendResumeId = payload.profile.id || state.backendResumeId;
      const docId = payload.profile.id || payload.doc_id;
      state.defaultFiles = [{
        name: payload.profile.source_file || `${docId}-能力图谱`,
        size: '本地已保存',
        type: 'pdf',
        uploaded: true,
        removable: true,
...
function forgetUploadedSession() {
  try {
    localStorage.removeItem('zhipu-uploaded-session');
  } catch (error) {
    // ignore
  }
}
...
function buildJobGraphData(job) {
  ...
  const links = [];
  const skillGroups = job.skill_groups;
  if (skillGroups && (skillGroups['专业技能'] || skillGroups['工具'] || skillGroups['软技能'])) {
    const groupMap = [
      ['专业技能', 'professional'],
      ['工具', 'tool'],
      ['软技能', 'soft']
    ];
    groupMap.forEach(([groupName, nodeType]) => {
      (skillGroups[groupName] || []).forEach((skillName) => {
        nodes.push({
          id: `req-${skillName}`,
          label: skillName,
          type: nodeType,
          level: 88,
          desc: `岗位能力图谱 · ${groupName}组`,
          evidence: []
        });
        links.push({ source: 'core', target: `req-${skillName}`, rel: groupName });
      });
    });
    return { nodes, links };
  }
...
function layoutGraph(rawNodes, links) {
  ...
  const typeAnchors = {
    ...
    professional: { angle: -1.2, spread: 1.3, radius: 270 },
    tool: { angle: 1.05, spread: 0.8, radius: 285 },
...
function radiusForNode(node) {
  ...
  if (node.type === 'professional') return 25;
  if (node.type === 'tool') return 22;
...
function renderMatchView() {
  ...
  if (detail) {
    const groups = [
      { label: '专业技能', value: detail.s_skill || 0, weight: detail.weights?.['专业技能'] ?? 0.8, color: 'var(--teal)' },
      { label: '工具', value: detail.s_tool || 0, weight: detail.weights?.['工具'] ?? 0.2, color: 'var(--blue)' },
      { label: '软技能', value: detail.s_soft || 0, weight: detail.weights?.['软技能'] ?? 0, color: 'var(--amber)' }
    ];
    $('#coverageBars').innerHTML = groups.map((group) => {
      const percent = Math.round(group.value * 100);
      return `
        <div class="score-row">
          <span>${group.label}</span>
          <div class="track"><i style="width:${percent}%;background:${group.color}"></i></div>
          <strong>${percent}%</strong>
        </div>
      `;
    }).join('');
    $('#matchFormula').innerHTML = `
      总分 = <strong>0.8 × 专业技能覆盖</strong> + <strong>0.2 × 工具覆盖</strong> + 0.0 × 软技能
      <span> · 软技能为通用词，仅作提示，不进入最终得分</span>
    `;
    $('#scoreFoot').innerHTML = `
      <div class="score-foot-row"><span>评分算法</span><strong>graph_match.py</strong></div>
      <div class="score-foot-row"><span>专业技能</span><strong>${Math.round((detail.s_skill || 0) * 100)}%</strong></div>
      <div class="score-foot-row"><span>工具</span><strong>${Math.round((detail.s_tool || 0) * 100)}%</strong></div>
      <div class="score-foot-row"><span>学历门槛</span><strong>已通过</strong></div>
    `;
  } else {
    $('#coverageBars').innerHTML = (job.dims || []).map((dim) => `
      <div class="score-row">
    ...
    `).join('');
    $('#matchFormula').textContent = '当前为前端内置示例口径，连接后端后显示 graph_match.py 真实覆盖得分。';
    $('#scoreFoot').innerHTML = '<div class="score-foot-row"><span>数据来源</span><strong>内置示例</strong></div>';
  }
  $('#gapList').innerHTML = job.gaps.map((gap) => {
    const label = gap.kind === 'required' ? '必需能力' : '可提升项';
    const className = gap.kind === 'required' ? 'required' : 'weak';
    const groupLabel = gap.group === '工具' ? '工具组' : gap.group === '软技能' ? '软技能组' : '专业技能组';
    return `
      <div class="gap-item">
        <span class="gap-name">
          ${esc(gap.name)}
          <span class="gap-type group">${esc(groupLabel)}</span>
...
function renderReport() {
  ...
  const detail = job.match || null;
  const algorithmRows = detail ? [
    { label: '专业技能覆盖率', value: `${Math.round((detail.s_skill || 0) * 100)}%`, note: `权重 ${detail.weights?.['专业技能'] ?? 0.8}` },
    { label: '工具覆盖率', value: `${Math.round((detail.s_tool || 0) * 100)}%`, note: `权重 ${detail.weights?.['工具'] ?? 0.2}` },
    { label: '软技能覆盖率', value: `${Math.round((detail.s_soft || 0) * 100)}%`, note: '通用词提示，不计总分' },
    { label: '学历门槛', value: detail.hard_filter_passed ? '通过' : '未通过', note: 'hard_gate' }
  ] : (job.dims || []).map((dim) => ({
    label: dim.label,
    value: dim.mine,
    note: `岗位目标 ${dim.target}`
  }));
  const report = `
    ...
    <section class="report-section">
      <h2>人岗图谱覆盖得分 <span>${detail ? '按 graph_match.py 清单口径计算' : '内置示例口径'}</span></h2>
      <div class="report-mini-grid">
        <table class="report-table">
          <thead><tr><th>能力维度</th><th>得分</th><th>权重 / 说明</th></tr></thead>
          <tbody>
        ...
        ${detail ? '<p class="report-formula">总分 = 0.8 × 专业技能覆盖 + 0.2 × 工具覆盖 + 0.0 × 软技能</p>' : ''}
      </div>
    </section>
    ...
    <section class="report-section">
      ...
      <table class="report-table" style="margin-top:10px">
        <thead><tr><th>差距项</th><th>能力组</th><th>性质</th><th>建议动作</th></tr></thead>
        <tbody>
          ${job.gaps.map((gap) => `
            <tr>
              <td>${esc(gap.name)}</td>
              <td>${esc(gap.group || '能力组')}</td>
...
function buildReportPayload() {
  ...
  return {
    ...
    skills: PERSONAL_SKILLS.slice(0, 14).map((skill) => ({
      name: skill.name,
      type: skill.type,
      level: skill.level
    })),
...
async function exportPdf() {
  ...
  const originalText = button?.innerHTML || '导出PDF';
  if (button) button.innerHTML = '正在生成PDF...';
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(error.error || `PDF生成失败 HTTP ${response.status}`);
    }
    const blob = await response.blob();
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = `求职适配诊断报告-${CANDIDATE.name || '用户'}.pdf`;
    ...
    console.error(error);
    if (button) button.innerHTML = '导出失败，请确认后端已启动';
  } finally {
    if (button && button.innerHTML !== '导出失败，请确认后端已启动') {
...
function renderUploadFiles() {
  const list = $('#fileList');
  list.innerHTML = state.defaultFiles.map((file, index) => fileRow(file, index)).join('');
  $('#fileCount').textContent = `${state.defaultFiles.length} 个文件`;
  const clearBtn = $('#clearUploadedBtn');
  if (clearBtn) clearBtn.hidden = state.defaultFiles.length === 0;
  applyIcons();
}

function refreshProfileUI() {
  const graphBadge = $('.nav-item[data-view="graph"] .nav-state');
  if (graphBadge) graphBadge.textContent = PERSONAL_SKILLS.length + PERSONAL_PROJECTS.length + 1;
  renderUploadFiles();
}

function fileRow(file, index = 0, progress = 100, status = '已完成') {
  const meta = fileTypeMeta(file.name);
  const ext = file.name.split('.').pop().toLowerCase();
  const kindText = ext === 'pdf' ? 'PDF简历' : ext === 'docx' ? 'Word简历' : ext === 'txt' ? '简历文本' : '能力图谱数据';
  return `
    <li class="file-row">
      ...
      <span class="file-row-main">
        ...
        <span class="file-meta">${esc(file.size || '')} · ${esc(kindText)}</span>
      </span>
      <span class="file-right">
        ...
        ${file.removable || file.uploaded ? `<button class="file-delete icon-btn" data-file-index="${index}" aria-label="删除${esc(file.name)}"><i data-icon="trash"></i></button>` : ''}
      </span>
    </li>
  `;
}

function addUploadRow(fileName, fileSize, type = 'pdf') {
  const list = $('#fileList');
  ...
  return {
    row,
    progressBar: $('.file-progress i', row),
    statusText: $('.file-status', row),
    metaText: $('.file-meta', row)
  };
}

function setUploadRowState(elements, progress, statusText, metaText, done = false) {
  elements.progressBar.style.width = `${progress}%`;
  elements.statusText.textContent = statusText;
  elements.statusText.className = done ? 'file-status done' : 'file-status busy';
  elements.metaText.textContent = metaText;
  $('#fileCount').textContent = `${$('#fileList').children.length} 个文件`;
}

async function uploadResumeFile(file) {
  const elements = addUploadRow(file.name, formatFileSize(file.size));
  setUploadRowState(elements, 12, '上传中', `${formatFileSize(file.size)} · 发送到后端`);
  try {
    setUploadRowState(elements, 28, '后端抽取中', '后端解析文件并调用LLM生成能力图谱，约需10-60秒');
    ...
    try {
      ...
      throw new Error(`后端返回异常：${raw.slice(0, 120)}`);
    }
    if (!response.ok || !payload.profile) {
      throw new Error(payload.error || payload.message || `上传失败 HTTP ${response.status}`);
    }
    setUploadRowState(elements, 72, '图谱生成中', `${file.name} · 图谱结构已返回`);
    const applied = applyBackendPayload(payload);
    if (!applied) throw new Error('后端返回的图谱数据不完整');
    rememberUploadedSession(payload);
    state.userUploaded = true;
    state.defaultFiles = [{
      name: file.name,
      ...
      uploaded: true,
      removable: true,
      docId: payload.doc_id || payload.profile?.id
    }];
    state.backendAvailable = true;
    refreshProfileUI();
    setUploadRowState(elements, 100, '图谱已生成', `${file.name} · 已完成能力抽取`, true);
    setTimeout(() => go('graph'), 350);
  } catch (error) {
    setUploadRowState(elements, 100, '上传失败', error.message || '解析失败');
  }
}

function supportedResumeFile(fileName) {
  const ext = fileName.split('.').pop().toLowerCase();
  return ['pdf', 'docx', 'txt'].includes(ext);
}

async function deleteFileRow(index) {
  const file = state.defaultFiles[index];
  if (!file) return;
  const shouldDeleteBackend = Boolean(file.uploaded && file.docId);
  if (shouldDeleteBackend) {
    try {
      const response = await fetch(
        `${API_BASE}/api/delete-resume?doc_id=${encodeURIComponent(file.docId)}`,
        { method: 'POST' }
      );
      const result = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(result.error || `删除失败 HTTP ${response.status}`);
    } catch (error) {
      const rowElement = $$('#fileList .file-row')[index];
      const statusText = rowElement ? $('.file-status', rowElement) : null;
      if (statusText) {
        statusText.textContent = '删除失败';
        statusText.className = 'file-status busy';
      }
      return;
    }
  }
  state.defaultFiles.splice(index, 1);
  renderUploadFiles();
  if (shouldDeleteBackend) {
    forgetUploadedSession();
    setTimeout(() => window.location.reload(), 250);
  }
}

async function clearUploadedFiles() {
  const uploaded = state.defaultFiles.filter((file) => file.uploaded && file.docId);
...
function bindEvents() {
  ...
  dropZone.addEventListener('drop', (event) => {
    ...
    const file = files[0];
    if (!file) return;
    if (!supportedResumeFile(file.name)) {
      const elements = addUploadRow(file.name, formatFileSize(file.size));
      setUploadRowState(elements, 100, '格式不支持', '请上传PDF、Word(.docx)或txt');
      return;
    }
  ...
  const clearBtn = $('#clearUploadedBtn');
  if (clearBtn) clearBtn.addEventListener('click', clearUploadedFiles);
...
async function init() {
  ...
  bindEvents();
  refreshProfileUI();
  const restored = restoreUploadedSession();
  refreshProfileUI();
  if (!restored) {
